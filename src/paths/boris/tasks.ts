...
export const borisDeltas: NamedDeltaTask[] = [
  ...deletedTasks.map((name) => <NamedDeltaTask>{
    name: name,
    delete: true,
  }),
  // Obtain Mayday immediately
  {
    ...
    replace: {
      completed: () =>
        !get("hasMaydayContract") || !have($item`MayDay™ supply package`)
    }
  },
  {
    name: "Misc/Trainset",
  ...
  tasks: [
    {
      name: "Skills",
      completed: () => (
        have($skill`Gourmand`) && have($skill`Banishing Shout`) && have($skill`Bifurcating Blow`)
    ...
    // Free Fights
    {
      ...
      ready: () => Witchess.have() && TrainSet.installed() && TrainSet.next() === TrainSet.Station.COAL_HOPPER,
      do: () => {
...
function getDesiredTrainsetConfig(): TrainSet.Cycle {
  ...
  if (myTurncount() <= 1)
    config.push(Station.TOWER_SEWAGE);  // hit with first CBB fight
  ...
  if (!config.includes(Station.TOWER_SEWAGE))
    config.push(Station.TOWER_SEWAGE);
