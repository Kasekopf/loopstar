...
const absorbTasks: AbsorbTask[] = [
  ...
  // Level 11: Hidden City
  {
    ...
    choices: () => {
      return {
        579: get("lastTempleAdventures") === myAscensions() ? 2 : 1,
        580: 3,
        581: 3,
      };
    },
  },
  {
    ...
    choices: () => {
      return {
        789: get("relocatePygmyJanitor") === myAscensions() ? 2 : 3,
      };
    },
...
export class AbsorbState {
  ...
  refresh() {
    this.absorbed = new Set<Monster>();
    this.reprocessed = new Set<Monster>();
    ...
    get(toTempPref("untrackedGooseReprocessed"))
      .split(",")
      .map((id) => parseInt(id))
      .filter((id) => id > 0)
      .map((id) => Monster.get(id))
    ...
    if (!get(toTempPref("museum"), false)) {
      visitUrl("museum.php?action=icehouse");
      set(toTempPref("museum"), true);
    }
  ...
  public getActiveTargets(location: Location): Set<Monster> {
    const result = new Set<Monster>();
    if (location) {
      // If we have teleportitis, everything is a possible target
      const zone = have($effect`Teleportitis`) ? undefined : location;
      for (const monster of this.remainingAbsorbs(zone)) result.add(monster);
      if (familiarWeight($familiar`Grey Goose`) >= 6)
        for (const monster of this.remainingReprocess(zone)) result.add(monster);
    }
    return result;
  }
}
  ...
  tasks: [
    ...
    ...absorbTasks.map((task): Task => {
      const result = {
        ...
        completed: () => !globalAbsorbState.hasReprocessTargets(task.do),
        ...task,
...
export const globalAbsorbState = new AbsorbState();
