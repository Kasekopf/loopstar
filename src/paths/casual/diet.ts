  ...
  tasks: [
    {
      ...
      completed: () =>
        myDaycount() > 1 ||
        (myFullness() >= args.casual.stomach && myInebriety() >= args.casual.liver),
      ready: () => myBasestat(myPrimestat()) >= 149 || myAdventures() <= 1,
      do: (): void => {
        if (have($item`astral six-pack`)) {
          use($item`astral six-pack`);
        }
        const MPA = args.casual.voa;
        ...
        const food = Math.max(args.casual.stomach - myFullness(), 0);
        const booze = Math.max(args.casual.liver - myInebriety(), 0);
        const spleen = Math.max(args.casual.spleen - mySpleenUse(), 0);
...
function itemPriority<T>(menuItems: readonly MenuItem<T>[]) {
